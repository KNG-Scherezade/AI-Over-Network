...
public class PlayerRails : MonoBehaviour
{
    ...
    {
        if (collider.tag == "WP" || (this.tag != "Player" && collider.tag == "GhostPen"))
        {
            this.GetComponentInParent<PlayerMovement>().current_node = collider.gameObject.GetComponent<NodeBehaviour>();
        }
    }
